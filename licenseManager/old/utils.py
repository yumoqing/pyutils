...
def extended_euclid_gcd_v1(a, b):
    ...
    """
    assert a or b, "bad arguments "
    assert a >= 0 and b >= 0, "bad arguments "
    if b == 0:
        return (a, 1, 0)
...
def extended_euclid_gcd_v2(a, b):
    ...
    """
    assert x or y, "bad arguments "
    assert x >= 0 and y >= 0, "bad arguments "
    if b == 0:
        return (a, 1, 0)
    q = abs(a % b)
    r = long(a / b)
...
def premier(a, b):
    ...
    """
    assert a or b, "both arguments are none "
    return gcd(a, b) == 1
